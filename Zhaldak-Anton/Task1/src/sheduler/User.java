...
public class User {
     ...
     *
     * @return User state. If state is <code>true</code> then user is active,
     * else user is passive.
     */
    ...
    /**
     * DISC: return number of position for insert in <code>event pool</code>.
     *       Try sort date event in ascending order (up) and text event in
     *       descending order (down). For example:<br>
     ...
     * @return Number of position for insert, or<br>
     *     -1 - input argument <code>event</code> is <code>null</code><br>
     *     -2 - <code>event pool</code> is null
